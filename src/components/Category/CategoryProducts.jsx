...
if (selectedMetal !== "all") {
  list = list.filter((p) => {
    const metal = (p.metal_type || "").toLowerCase().trim();
          ...
          {products.length === 0 ? (
            ...
            <div className={styles.grid}>
              {products.map((p) => {
                ...
                const stock = p.remaining_stock ?? p.stock ?? undefined; // supplier me stock nahi aata, seller me aa sakta hai
                const isOutOfStock = stock !== null && stock !== undefined && Number(stock) === 0;
                const isComingSoon = stock === null || undefined;
